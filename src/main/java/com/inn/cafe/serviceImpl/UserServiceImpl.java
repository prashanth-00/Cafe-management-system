    ...
    try {
      if (validaSignUpMap(requestMap)) {
        ...
        if (Objects.isNull(user)) {
          // Set new user's status to false (it defaults to null)
  ...
  public ResponseEntity<String> login(Map<String, String> requestMap) {
    ...
    try {
      ...
      if (auth.isAuthenticated()) {
        // if user is approvedd by admin then generates token
        if (customerUserDetailsService.getUserDatails().getStatus().equalsIgnoreCase("true")) {
          return new ResponseEntity<String>("{\"token\":\"" + jwtUtil.generateToken(
              customerUserDetailsService.getUserDatails().getEmail(),
              customerUserDetailsService.getUserDatails().getRole()) + "\"" + ",\"role\":\""
              + customerUserDetailsService.getUserDatails().getRole() + "\"}",
              HttpStatus.OK);
        } else { // user is not approved by admin and the message will be rendered
  ...
  public ResponseEntity<String> update(Map<String, String> requestMap) {
    try {
      if (jwtFilter.isAdmin()) {
        ...
        if (optional.isPresent()) {// changed isEmpty to ! isPresent
